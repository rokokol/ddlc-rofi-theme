...
Скруглённые полупрозрачные строки на фоне из игры в горошек, в светлом и тёмном варианте, и простая переключалка между ними
...
- [Установка](#установка)
  ...
  - [Шрифтов здесь нет](#шрифтов-здесь-нет)
- [Переключалка](#переключалка)
- [Как rofi её находит](#как-rofi-её-находит)
- [Тесты](#тесты)
- [Структура](#структура)
- [Лицензия](#лицензия)
...
> Строки здесь — [rofi-wooordhunt](https://github.com/rokokol/rofi-wooordhunt)
...
На скриншотах — **Doki**, шрифт из DDLC, а строки набраны [DepartureMono](https://departuremono.com). Ни того, ни другого в комплекте нет: тема не должна ставить шрифты, да и проблемы с копирайтом. Из коробки берутся `sans-serif` и `monospace`, а `font` и `monoFont` наводятся на то, что есть на ПК
